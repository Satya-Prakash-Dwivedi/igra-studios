...
import { Trash2, Phone, Calendar, FileText, Upload, Link2 } from "lucide-react";

function ConsultationForm({ onRemove, instanceId, service }: any) {
    // Local state
    ...
    const durationOptions = [
        { value: "15", label: "15 Minutes", credits: 100 },
        { value: "30", label: "30 Minutes", credits: 200 },
        { value: "60", label: "60 Minutes", credits: 400 },
    ];

    // Find correct credit number for selected duration
    const selectedCredits =
        durationOptions.find((d) => d.value === duration)?.credits ||
        service.credits;
            ...
            <div className="px-8 py-8 space-y-8">
                    ...
                    <div className="flex gap-4 flex-wrap">
                        {durationOptions.map((opt) => (
                            <button
                                ...
                                className={`px-6 py-3 rounded-full border text-sm font-semibold transition-all duration-200 backdrop-blur-sm
                  ${duration === opt.value
                                        ? "bg-gradient-to-r from-red-500 to-red-600 border-red-400 text-white scale-105"
                                        : "bg-gray-800/50 border-gray-600/50 text-gray-300 hover:border-red-400/50 hover:bg-gray-700/50"
                                    }`}
                            >
                                {opt.label}
                            </button>
                ...
                <div className="space-y-3">
                    <div className="flex items-center space-x-2">
                        ...
                        <label className="text-lg font-semibold bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent">
                            Topics to Discuss <span className="text-gray-400 font-normal">(one per line)</span>
                        </label>
                    </div>
                    <textarea
                        rows={3}
                ...
                <div className="space-y-3">
                    ...
                    <textarea
                        ...
                        placeholder="Any specific area to focus on?"
                    />
                </div>

                {/* Upload Asset / Reference Link Side by Side */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="relative">
                            ...
                            <div className="flex items-center justify-center w-full h-24 px-6 py-4 border-2 border-dashed border-gray-600/50 rounded-xl bg-gray-800/30 hover:border-red-400/50 hover:bg-gray-800/50 transition-all cursor-pointer">
                                <div className="text-center">
                                    ...
                                    <p className="text-sm text-gray-500">Any supporting file, doc, design, etc.</p>
                                </div>
                            </div>
                        </div>
                        {uploadedFile && (
                    ...
                    <div className="space-y-3">
                        ...
                        <input
                            ...
                            placeholder="Paste meeting doc, Figma, or asset link"
                        />
                        <p className="text-sm text-gray-400 ml-1">
                            Google Doc, Figma, Dropbox, etc.
                        </p>
                    </div>
                </div>
            </div>
